...
class ShareLinkPlugin(SoCoPlugin):
    ...
    def add_share_link_to_queue(self, uri, position=0, as_next=False, **kwargs):
        ...
        Args:
            ...
            **kwargs: any keyword arguments. If ``playlist_title`` is specified
                when adding a Spotify playlist, this will be used as the title
                of the playlist container. If not specified, the playlist title
                will be empty.
        ...
        for service in self.services:
            if service.canonical_uri(uri):
                ...
                if share_type == "playlist" and isinstance(service, SpotifyShare):
                    playlist_title = kwargs.pop("playlist_title", "")
                    metadata_template = (
