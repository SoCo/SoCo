...
def get_interfaces():
    """ Get all network interfaces addresses on the local machine.

    Return an list of network addresses on the local machine.
    """
    all_addr = []
    for interface in netifaces.interfaces():
        if_addr = netifaces.ifaddresses(interface)
        # Only first addr for each physical interface
        all_addr.append(if_addr[netifaces.AF_INET][0]['addr'])
    return all_addr


def _send_discover(interface_address=None):
    """ Send sonos multicast discover event on a network interface
    """
    ...
    _sock.sendto(really_utf8(player_search), (mcast_group, mcast_port))
    _sock.sendto(really_utf8(player_search), (mcast_group, mcast_port))
    _sock.sendto(really_utf8(player_search), (mcast_group, mcast_port))
...
def discover(interface_addresses=None, timeout=1, include_invisible=False):
    """ Discover Sonos zones on the network.

    Return an set of visible SoCo instances for each zone found.
    ´interface_addresses` can be a list of network addresses on
    which to discover, default only the first interface will be disovered.
    Use the function get_interfaces for a list of all intefaces.
    ...
    which return `[]` if no zones found.
    ...
    # Make the timeout true
    timeout = float(timeout)/len(interface_addresses)

    # Probe all interfaces
    for interface_address in interface_addresses:
        _sock = _send_discover(interface_address)
        ...
        while True:
            ...
            #
            # Here is a sample response from a real Sonos device (actual
            ...
            if response:
                ...
                # It is much more efficient to rely upon the Zone Player's
                # ability to find the others, than to wait for query
                # responses from them ourselves.
                zone = config.SOCO_CLASS(addr[0])
                if include_invisible:
                    LOGGER.debug("Found {0}".format(zone.all_zones))
                    all_zones += zone.all_zones
                    break
                else:
                    LOGGER.debug("Found {0}".format(zone.visible_zones))
