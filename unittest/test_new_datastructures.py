...
def assert_xml_equal(left, right, explain=None):
    ...
    def _build_explanation(left, right, explain):
        ...
        if left.text != right.text:
            explain.append('text for element <%s>: %r != %r'%(left.tag, left.text,
                right.text))
        if left.tail != right.tail:
            explain.append('tail for element <%s>: %r != %r'%(left.tag, left.text,
                right.text))
...
class TestDidlObject():
    ...
    def test_create_didl_object_with_disallowed_params(self):
        with pytest.raises(ValueError) as excinfo:
            ...
            parent_id='pid', item_id='iid', bad_args='other')
